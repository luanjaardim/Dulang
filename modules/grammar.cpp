...
Element *getNextElement(Token tk) {
  if(tk.type == TK_CAP_NAME) {

    if(tk.text == "Name") return new Element(ElementValue(TK_NAME));
    else if(tk.text == "Number") return new Element(ElementValue(TK_INT));
    else if(tk.text == "String") return new Element(ElementValue(TK_STR));
    else if(tk.text == "Char") return new Element(ElementValue(TK_CHAR));
    else if(tk.text == "CCode") return new Element(ElementValue(TK_INLINE_C));
    // else if(tk.text == "Block") return new Element(ElementValue(VAL_BLOCK));
    else if(tk.text == "NewLine") return new Element(ElementValue(TK_NEW_LINE));
    // else if(tk.text == "Indent") return new Element(ElementValue(VAL_INDENT));

  } else if(tk.type == TK_NAME) {
    return new Element(InnerElement(tk.text));
  }
  cerr << "Error: Could not get the next element\n";
  exit(1);
}

void Grammar::extractPatterns(Lexer *lexer) {
  string key;
  Token tk;
  while((tk = lexer->nextToken()).type != TK_EOF) {
    continue_without_getting_next:
    if(tk.type == TK_TYPE_DEREF) { // @
      tk = lexer->nextToken();
      if(tk.type != TK_NAME) {
        printf("Error: expected a name followed by a '->' after @\n");
        exit(1);
      }
      key = tk.text;
      this->rules[key] = {};
    } else if(tk.type == TK_END_BAR || tk.type == TK_TYPE_FN_ARROW) {
      this->rules[key].push_back(Rule());
      while((tk = lexer->nextToken()).type != TK_NEW_LINE &&  tk.type != TK_END_BAR && tk.type != TK_EOF) {
        this->rules[key].back().elements.push_back(getNextElement(tk));
      }
      goto continue_without_getting_next;
    }
  }
}

void Grammar::calculateFirsts() {
  for( auto key : this->rules ) {
    firsts[key.first] = {};
  }
  bool changed;
  string key_str;
  ...
  do {
    ...
    for( auto key : this->rules ) {
      key_str = key.first;
      tmp.clear();
      for( auto rule : key.second ) {
        for(int i = 0; i < rule.elements.size() - 1; i++) {
          elem = rule.elements[i];
          if(elem->type == TEXT) {
            tmp.insert(elem->text.text);
          }
        }
      }
      if(firsts[key_str] != tmp) {
        firsts[key_str].insert(tmp.begin(), tmp.end());
        changed = true;
      }
    }
  } while(changed);
...
//   bool failed = false;
//
//   debug printf("Parsing every pattern of %s\n", ps.key.c_str());
...
//     }
//     debug {
//       printf("pattern %d, failed? %s\n", (int)pat_idx, failed ? "yes" : "no");
//       if(!failed && tmpSteps.steps.empty()) printf("\tIt's empty\n");
//     }
//
//     if(!failed) {
...
//
//   debug {
//     printf("every possible step to take for the pattern %s\n", ps.key.c_str());
//     for( auto steps : possibleSteps ) {
//       for( auto step : steps.steps ) {
//         printf("Key: %s\n", step.key.c_str());
//         printf("Start at line %d and elem %d.\n", (int)step.start.l, (int)step.start.e);
//         printf("End at line %d and elem %d.\n", (int)step.end.l, (int)step.end.e);
//       }
//       printf("--------------------------\n");
//     }
//     printf("==========================\n");
...
//
//   if(ps.key == "stmt") {
//     printf("Error: parsing error from line %d, column %d\n", (int)ps.start.l, (int)ps.start.e);
//     printf("Expected a statement\n");
//   }
//
//   return NULL;
// }
